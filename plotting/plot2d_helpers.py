...
def point_inside_eto_projection(E: float, T: float, O: float, bounds: Dict[str, float]) -> bool:
    return (
...
def segment_colors_for_solution_2d(sol, bounds: Dict[str, float]) -> list[str]:
    ...
    return [
        INSIDE_COLOR
        if point_inside_eto_projection(float(E[i + 1]), float(T[i + 1]), float(O[i + 1]), bounds)
        else OUTSIDE_COLOR
        for i in range(len(sol.t) - 1)
    ]


def add_et_background(
    ax,
    *,
    ...
    if show_box:
        rect = Rectangle(
            (bounds["E_min"], bounds["T_min"]),
            bounds["E_max"] - bounds["E_min"],
...
def save_et_animation(
    ...
    *,
    ...
    for sol in solutions:
        ...
        T0 = float(sol.y[1, 0])
        O0 = float(sol.y[3, 0])
        inside0 = point_inside_eto_projection(E0, T0, O0, bounds)
    ...
    ax.set_title(
        f"{label}\nAnimated ensemble in the (E, T) phase plane",
        fontsize=12,
        fontweight="bold",
    )
    ...
    def update(frame: int):
        for sol, lc, point, seg_colors in zip(solutions, line_collections, points, segment_color_lists):
            ...
            point.set_color(
                INSIDE_COLOR if point_inside_eto_projection(E_now, T_now, O_now, bounds) else OUTSIDE_COLOR
            )
