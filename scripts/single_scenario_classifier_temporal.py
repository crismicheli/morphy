...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate both a 3D trajectory animation and a classifier-labeled 3D plot for one scenario."
    )
...
def get_axes_limits():
    return max(2.0, DEFAULTBOUNDS["E_max"] * 1.08), max(1.6, DEFAULTBOUNDS["T_max"] * 1.08), 1.4
...
def save_classifier_plot(result: dict, scenario_cfg: dict, args: argparse.Namespace, output_path: Path) -> None:
...
def save_trajectory_animation(result: dict, args: argparse.Namespace, output_path: Path) -> None:
    ...
    segment_color_lists = [segment_colors_for_solution(sol, DEFAULTBOUNDS) for sol in solutions]
    line_collections = []
    points = []
    for sol in solutions:
        E0 = float(sol.y[2, 0])
        T0 = float(sol.y[1, 0])
        O0 = float(sol.y[3, 0])
        inside0 = point_inside_eto_box(E0, T0, O0, DEFAULTBOUNDS)
    ...
    c_text = ax.text2D(0.98, 0.06, f"C ({args.c_stat}) = 0.000", transform=ax.transAxes, va="bottom", ha="right", fontsize=10, bbox=dict(boxstyle="round,pad=0.35", fc="white", ec="0.8", alpha=0.94))
    ...
    def update(frame: int):
        t_now = float(solutions[0].t[frame])
        ...
        for sol, lc, point, seg_colors in zip(solutions, line_collections, points, segment_color_lists):
            ...
            point.set_color(INSIDE_COLOR if point_inside_eto_box(E_now, T_now, O_now, DEFAULTBOUNDS) else OUTSIDE_COLOR)
...
def main() -> None:
    ...
    animation_path = out_dir / f"{prefix}_eto_animation.gif"
    classifier_plot_path = out_dir / f"{prefix}_{args.classifier_type}_taxonomy_3d.png"

    save_trajectory_animation(result, args, animation_path)
    save_classifier_plot(result, scenario, args, classifier_plot_path)

    print(f"Scenario: {result['label']}")
    print(f"Classifier type: {args.classifier_type}")
    print(f"Animation: {animation_path}")
    print(f"Classifier plot: {classifier_plot_path}")
